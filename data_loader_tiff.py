%%writefile /kaggle/working/step2to4_train_validate_inference/loader/data_loader_tiff.py
# %load /kaggle/working/step2to4_train_validate_inference/loader/data_loader_tiff.py
import random
...
import sys
sys.path.append('/kaggle/working/step2to4_train_validate_inference/tnscui_utils/')

from img_mask_aug import data_aug
from TNSUCI_util import char_color
...
class TifFolder(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    Transform.append(T.Resize((self.image_size, self.image_size), interpolation=Image.BICUBIC))
    Transform_GT.append(T.Resize((self.image_size, self.image_size), interpolation=Image.NEAREST))
    ...
    for image, mask in zip(tiff_images, tiff_masks):
      ...
      #print(torch.unique(tiff_masks_prcd[0])) 
      #print('##########')
      #print(Transform(image).shape, Transform_GT(mask).shape)
      #print('#########')
